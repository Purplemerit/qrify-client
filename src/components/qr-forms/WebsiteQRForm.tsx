...
const WebsiteQRForm = ({ onGenerate }: WebsiteQRFormProps) => {
  ...
  const addLanguage = () => {
    setLanguages([...languages, { id: Date.now().toString(), language: "", url: "" }])
  }
  ...
  const updateLanguage = (id: string, field: 'language' | 'url', value: string) => {
  ...
  const addCountry = () => {
    setCountries([...countries, { id: Date.now().toString(), country: "", websiteName: "" }])
  }
  ...
  const updateCountry = (id: string, field: 'country' | 'websiteName', value: string) => {
              ...
              {countries.map((country) => (
                <div key={country.id} className="flex gap-2 items-start">
                  <Input
                    ...
                    onChange={(e) => updateCountry(country.id, 'country', e.target.value)}
                    className="flex-1"
                  />
                  <Input
                    placeholder="Website name"
                    value={country.websiteName}
                    onChange={(e) => updateCountry(country.id, 'websiteName', e.target.value)}
                    className="flex-1"
                  />
                  <Button
                    variant="outline"
